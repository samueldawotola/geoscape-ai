...
export function CollectionSurfer({ items = ITEMS, variant = "magnetic", title }: CollectionSurferProps) {
    ...
    return (
        <div className="relative min-h-screen text-white w-full">
            ...
            >
                ...
                {/* UI Overlays */}
                {title && (
                    <div className="absolute top-20 left-[3vw] z-40 pointer-events-none mix-blend-difference">
                        <h1 className="font-heading font-bold text-3xl md:text-5xl leading-[0.9] tracking-tight">
                            {title}
                            <span className="ml-2 align-top text-[0.5em] font-mono tabular-nums">
                                ({items.length})
                            </span>
                        </h1>
                    </div>
...
function Card({
    ...
    const content = (
        <>
            ...
            {/* Place name caption */}
            <div className="absolute inset-x-0 top-0 bg-gradient-to-b from-black/85 via-black/30 to-transparent px-4 pb-10 pt-3 pointer-events-none">
